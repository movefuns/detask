...
interface Props {
     ...
     refetch: any; // 根据 useNetworkVariable 的类型进行定义
     setAllObjects: any; // 根据 useNetworkVariable 的类型进行定义
     setStringArray: any; // 根据 useNetworkVariable 的类型进行定义
     setCursor: any; // 根据 useNetworkVariable 的类型进行定义
}

const MoveButton : React.FC<Props> = ({setMoveNow,setTaskNow,refetch,setAllObjects,setStringArray,setCursor}) => {
    ...
    const handleOpenDialog = () => {
        ...
        setCursor(null);
        refetch();
        console.log("set move now !  refetch now !!");
    };
    ...
    return (
        <div>
            <button type="button" className='btMove' onClick={handleOpenDialog}>$MOVE</button>
            {/* {showDialog && (
